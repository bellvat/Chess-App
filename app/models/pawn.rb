...
# ----- Otherwise -------
  ...
  def en_passant?(new_x_coord, new_y_coord)
    ...
    return false if (new_y_coord != y_coord + 1 && !white?) || (new_y_coord != y_coord - 1 && white?)
    return false unless (new_x_coord == x_coord + 1) || (new_x_coord == x_coord - 1)
    return false unless (new_y_coord == 3 && white?) || (new_y_coord == 6 && !white?)
